...
class StarredEmailsCommand extends Command
{
    protected $signature = 'gmail:starred';
    protected $description = 'List starred emails from Gmail';

    public function handle(GmailService $gmailService)
    {
        intro('Starred Emails');

        try {
            $emails = spin(fn() => $this->fetchEmails($gmailService), 'Fetching starred emails...');

            if (empty($emails)) {
                note('No starred emails found.');
                return;
            }
    ...
    {
        $messages = $gmailService->listMessages(20, 'is:starred');
