...
class UnreadEmailsCommand extends Command
{
    protected $signature = 'gmail:unread';
    protected $description = 'List unread emails from Gmail';
    ...
    public function handle(GmailService $gmailService)
    {
        ...
        try {
            $emails = spin(fn() => $this->fetchEmails($gmailService), 'Fetching unread emails...');

            if (empty($emails)) {
                note('No unread emails found.');
                return;
            }
    ...
    {
        $messages = $gmailService->listMessages(20, 'is:unread');
