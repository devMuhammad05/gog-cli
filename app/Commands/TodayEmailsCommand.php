...
class TodayEmailsCommand extends Command
{
    protected $signature = 'gmail:today';
    protected $description = 'List emails from today';

    public function handle(GmailService $gmailService)
    {
        intro('Today\'s Emails');

        try {
            $emails = spin(fn() => $this->fetchEmails($gmailService), 'Fetching today\'s emails...');

            if (empty($emails)) {
                note('No emails from today found.');
                return;
            }
    ...
    private function fetchEmails(GmailService $gmailService): array
    {
        ...
        $messages = $gmailService->listMessages(20, $query);
