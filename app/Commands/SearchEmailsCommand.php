...
class SearchEmailsCommand extends Command
{
    protected $signature = 'gmail:search {query : The search query}';
    protected $description = 'Search Gmail messages using Gmail search operators';
    ...
    public function handle(GmailService $gmailService)
    {
        ...
        try {
            $emails = spin(fn() => $this->fetchEmails($gmailService, $query), "Searching for: $query");

            if (empty($emails)) {
                note('No emails found matching your query.');
                return;
            }
    ...
    private function fetchEmails(GmailService $gmailService, string $query): array
    {
        $messages = $gmailService->listMessages(10, $query);
