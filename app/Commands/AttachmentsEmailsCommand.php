...
class AttachmentsEmailsCommand extends Command
{
    protected $signature = 'gmail:attachments';
    protected $description = 'List emails with attachments from Gmail';

    public function handle(GmailService $gmailService)
    {
        intro('Emails with Attachments');

        try {
            $emails = spin(fn() => $this->fetchEmails($gmailService), 'Fetching emails with attachments...');

            if (empty($emails)) {
                note('No emails with attachments found.');
                return;
            }
    ...
    {
        $messages = $gmailService->listMessages(20, 'has:attachment');
